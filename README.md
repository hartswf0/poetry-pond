...
## The typed causal-world line (servers)

Each runs on Node 20+, no dependencies, and accepts an OpenAI API key at runtime
from its settings panel — the key is held by the local server only (memory +
`.runtime-config.json`, git-ignored) and never returned to the browser. All
three fall back to a fully local typed engine without a key.
