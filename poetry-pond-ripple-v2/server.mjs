...
// The key lives only on the server: environment first, else the runtime
// config saved from the settings panel. It is never echoed to the browser.
const RUNTIME_CONFIG_PATH = path.join(__dirname, '.runtime-config.json');
let OPENAI_API_KEY = process.env.OPENAI_API_KEY || '';
let keySource = OPENAI_API_KEY ? 'env' : 'none';
let PLANNER_MODEL = process.env.OPENAI_PLANNER_MODEL || process.env.OPENAI_MODEL || 'gpt-5.6-sol';
let WRITER_MODEL = process.env.OPENAI_WRITER_MODEL || process.env.OPENAI_MODEL || 'gpt-5.6-sol';
...
async function loadRuntimeConfig() {
  try {
    ...
    if (!process.env.OPENAI_PLANNER_MODEL && !process.env.OPENAI_MODEL && raw.plannerModel) PLANNER_MODEL = String(raw.plannerModel);
    if (!process.env.OPENAI_WRITER_MODEL && !process.env.OPENAI_MODEL && raw.writerModel) WRITER_MODEL = String(raw.writerModel);
  } catch { /* no runtime config yet */ }
}

async function saveRuntimeConfig() {
  const payload = { apiKey: keySource === 'env' ? '' : OPENAI_API_KEY, plannerModel: PLANNER_MODEL, writerModel: WRITER_MODEL };
  await writeFile(RUNTIME_CONFIG_PATH, JSON.stringify(payload, null, 2), { mode: 0o600 });
}
...
function statusPayload() {
  return {
    ...
    model: WRITER_MODEL,
    contextMode: 'explicit-world-state',
    store: false
  };
}

// Server-side context log: one entry per turn, inspectable from the client
// and appended to logs/context-log.jsonl. The API key is never logged.
const CONTEXT_LOG_MAX = 200;
const contextLog = [];
const LOG_DIR = path.join(__dirname, 'logs');
const LOG_FILE = path.join(LOG_DIR, 'context-log.jsonl');
function logContext(entry) {
  const rec = { at: new Date().toISOString(), ...entry };
  contextLog.push(rec);
  if (contextLog.length > CONTEXT_LOG_MAX) contextLog.shift();
  mkdir(LOG_DIR, { recursive: true })
    .then(() => appendFile(LOG_FILE, JSON.stringify(rec) + '\n'))
    .catch(() => {});
}
...
async function callStructured({ model, instructions, input, schema, name, maxOutputTokens = 1400, signal }) {
...
async function proposeEvent(world, input, useOpenAI, signal) {
  const local = proposeLocalEvent(world, input);
  if (local.status === 'accepted' || local.clarification?.code !== 'unsupported-action' || !useOpenAI || !OPENAI_API_KEY) return { proposal: local, plannerUsage: null };
  const context = {
    participant_line: input.text,
    mask: input.mode,
    ontology: TYPE_SCHEMAS,
    world: worldDigest(world)
  };
  try {
    const call = await callStructured({ model: PLANNER_MODEL, instructions: plannerInstructions, input: context, schema: plannerSchema, name: 'poetry_pond_event_proposal', maxOutputTokens: 1700, signal });
    return { proposal: mapPlannerProposal(world, input, call.parsed), plannerUsage: { model: call.model, responseId: call.responseId, usage: call.usage } };
  } catch (error) {
    return { proposal: local, plannerUsage: { model: PLANNER_MODEL, responseId: null, usage: null, error: error.message } };
  }
}

async function renderEvent(nextWorld, event, relevantEvents, useOpenAI, signal) {
  const local = renderLocalTurn(nextWorld, event, relevantEvents);
  if (!useOpenAI || !OPENAI_API_KEY) return { rendered: local, writerUsage: null };
  const input = {
  ...
  try {
    const call = await callStructured({ model: WRITER_MODEL, instructions: writerInstructions, input, schema: writerSchema, name: 'poetry_pond_validated_render', maxOutputTokens: 1100, signal });
    const raw = {
      ...
      generator: 'openai-validated-writer'
    };
    const checked = validateRenderedTurn(nextWorld, event, relevantEvents, raw);
    if (!checked.ok) throw new Error(checked.errors.join(' '));
    return { rendered: { ...checked.rendered, generator: 'openai-validated-writer' }, writerUsage: { model: call.model, responseId: call.responseId, usage: call.usage } };
  } catch (error) {
    return { rendered: local, writerUsage: { model: WRITER_MODEL, responseId: null, usage: null, error: error.message } };
  }
}
  ...
  try {
    ...
    if (req.method === 'POST' && url.pathname === '/api/llm-key') {
      ...
      if (key === '') {
        ...
        return json(res, 200, { ...statusPayload(), cleared: true });
      }
      ...
      OPENAI_API_KEY = key;
      keySource = 'runtime';
      if (typeof body.plannerModel === 'string' && body.plannerModel.trim()) PLANNER_MODEL = body.plannerModel.trim().slice(0, 80);
      if (typeof body.writerModel === 'string' && body.writerModel.trim()) WRITER_MODEL = body.writerModel.trim().slice(0, 80);
      await saveRuntimeConfig();
      return json(res, 200, statusPayload());
    }
    if (req.method === 'GET' && url.pathname === '/api/context-log') {
      const limit = Math.max(1, Math.min(CONTEXT_LOG_MAX, Number(url.searchParams.get('limit')) || 60));
      return json(res, 200, { entries: contextLog.slice(-limit), total: contextLog.length, file: 'logs/context-log.jsonl' });
    }
    if (req.method === 'POST' && url.pathname === '/api/turn') {
      ...
      const { proposal, plannerUsage } = await proposeEvent(world, input, useOpenAI, controller.signal);
      if (proposal.status !== 'accepted') {
        logContext({
          kind: 'turn', requestId: body.request_id || null, text: input.text,
          status: proposal.status, proposer: proposal.source || 'local-proposer',
          clarificationCode: proposal.clarification?.code || null,
          plannerError: plannerUsage?.error || null
        });
        return json(res, 200, { ...proposal, plannerUsage });
      }
      ...
      const { rendered, writerUsage } = await renderEvent(nextWorld, event, relevantEvents, useOpenAI, controller.signal);
      const checked = validateRenderedTurn(nextWorld, event, relevantEvents, rendered);
      if (!checked.ok) throw new Error(checked.errors.join(' '));
      logContext({
        kind: 'turn', requestId: body.request_id || null, text: input.text,
        status: 'accepted', proposer: proposal.source || 'local-proposer',
        generator: checked.rendered.generator || rendered.generator || 'local-typed',
        eventId: event.id, changeCount: event.changes.length,
        changes: event.changes.map(change => `${change.entityId}.${change.path}: ${JSON.stringify(change.before)} → ${JSON.stringify(change.after)}`),
        relevantEventIds: relevantEvents.map(item => item.id),
        plannerError: plannerUsage?.error || null, writerError: writerUsage?.error || null,
        usage: {
          planner: plannerUsage?.usage ? { input: plannerUsage.usage.input_tokens, output: plannerUsage.usage.output_tokens } : null,
          writer: writerUsage?.usage ? { input: writerUsage.usage.input_tokens, output: writerUsage.usage.output_tokens } : null
        }
      });
      return json(res, 200, {
        status: 'accepted',
...
server.listen(PORT, HOST, () => {
  console.log(`Poetry Pond Ripple V2 listening on http://${HOST}:${PORT}`);
  console.log(`OpenAI planner: ${OPENAI_API_KEY ? PLANNER_MODEL : 'disabled'}; writer: ${OPENAI_API_KEY ? WRITER_MODEL : 'disabled'} (key source: ${keySource})`);
});
